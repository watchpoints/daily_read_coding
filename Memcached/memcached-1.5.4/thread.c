...
item *item_get(const char *key, const size_t nkey, conn *c, const bool do_update)
{
    ...
    uint32_t hv;
    hv = hash(key, nkey);
    item_lock(hv);////获得分段锁信息，如果未进行扩容，则item的hash表是多个hash桶共用同一个锁，即是分段的锁
    it = do_item_get(key, nkey, hv, c, do_update); ////执行get操作
    item_unlock(hv);
    return it;
}
 ...
 * http://chenzhenianqing.com/articles/1223.html
...
void memcached_thread_init(int nthreads, void *arg)
{
    ...
    for (i = 0; i < nthreads; i++)
    {
        create_worker(worker_libevent, &threads[i]); //create_worker 异步的
    }
